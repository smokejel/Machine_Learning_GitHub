...
class Blob:
    ...
    def move(self, x=False, y=False):
        if not x:
            self.x += np.random.randint(-1, 2) # can move -1, 0 or 1 (up to 2 does not include 2)
        else:
            self.x += x

        if not y:
            self.y += np.random.randint(-1, 2) # can move -1, 0 or 1 (up to 2 does not include 2)
        else:
            self.y += y
...
with open(f"qtable-{int(time.time())}.pickel", "wb") as f:
